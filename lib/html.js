...
class HTMLRewriter {
  ...
  process(source, config = {}) {
    ...
    iterate(ast, (node) => {
      ...
      // Inject lazy loading for images and iframes if missing
      if (
      ...
      // Default alt text injection if missing for images
      if (node.tagName === "IMG" && !node.hasAttribute("alt")) {
        node.setAttribute("alt", "Image");
      }

      // Handle <picture> element children by recursively processing sources
          ...
          if (child.tagName === "SOURCE") {
            child.attrs.forEach((attr) => {
              const handler = this.attributeRoute({
                ...attr,
                node: new Parse5Wrapper(child),
              });
              if (handler === "srcset") {
                child.setAttribute(`corrosion-${attr.name}`, attr.value);
      ...
      switch (node.tagName) {
        case "STYLE":
          ...
          break;
        case "TITLE":
          if (this.ctx.config.title) node.textContent = this.ctx.config.title;
          break;
        case "SCRIPT":
          if (
        ...
        case "LINK":
          ...
          if (
            rel &&
            (rel.toLowerCase() === "icon" ||
              rel.toLowerCase() === "shortcut icon")
          ) {
            if (this.ctx.config.favicon) {
              node.setAttribute("href", this.ctx.config.favicon);
            }
          }
          break;
      }
      node.attrs.forEach((attr) => {
        ...
        switch (handler) {
          ...
          case "delete":
            ...
            break;
          case "lazyload":
            // Always set loading=lazy for lazyload attribute
            node.setAttribute("loading", "lazy");
            break;
        }
      });
    ...
    const nodes = [
      ...
      },
      {
        ...
        attrs: [
          ...
          {
            name: "href",
            value: this.ctx.config.favicon || "/favicon.ico",
          },
        ],
      },
      {
